...
* **< 0** — the probe is *worse* than knowing nothing, which on held-out data means it fitted
  its own training set. See the `MLP < linear` tripwire in `pim.probes`.
...
def r2(pred: np.ndarray, y: np.ndarray, train_mean: np.ndarray) -> float:
    ...
    Identical to `probe_skill_regression`; kept under its native name so regression
    tables can label the column R² while the cross-environment axis says skill.
    """
