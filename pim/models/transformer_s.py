"""Transformer-S — the small (~3.2M) canonical architecture, ours.

Ported verbatim 2026-08-31 from `pim/world_models/transformer/model.py`
(`TransformerModel` → `TransformerS`; state-dict keys unchanged, so every existing
checkpoint loads). Two task heads share one body:

  * `TransformerS` — regression: `Linear(obs_res → d_model)+ReLU` in, MSE out.
    The discworld world model.
  * `TransformerSTokens` — classification: `Embedding(vocab → d_model)` in,
    move logits out, cross-entropy. The Othello form. Exactly three things differ
    (encoder, decoder, loss); everything else — RoPE, pre-norm blocks, banded-causal
    attention, the `_run` edit hook — is shared by inheritance, not by copy.
...
class TransformerS(nn.Module):
    ...
    def n_layers(self) -> int:
        """Block count (protocol surface; residual points = n_layers + 1)."""
...
class TransformerSTokens(TransformerS):
    """Transformer-S with a token embedding in and move logits out.

    Ported verbatim from ``ours_on_othello/model.py`` (``OthelloTransformer``). Exactly
    three things change from the regression form:

        encoder   Linear(obs_res, d_model) + ReLU  ->  nn.Embedding(vocab, d_model)
        decoder   Linear(d_model, obs_res)         ->  Linear(d_model, vocab)
        loss      MSE on the next observation      ->  cross-entropy on the next move

    No ReLU on the embedding: an embedding lookup is already a free parameter per
    token, so a ReLU would only zero half of it (Sevan's call, 2026-08-21).

    Window-size note: a window-16 model CAN see a 60-move game — stacking widens the
    receptive field, so ``state_span = n_layers*(window-1)+1 = 4*15+1 = 61``, covering
    a full game exactly. What the band costs is directness (their minGPT attends to any
    earlier move in one hop; this must route through banded hops), which is an
    architectural difference under study, not a bug.
    """

    def __init__(self, cfg: ModelConfig, vocab: int = 61) -> None:
        super().__init__(cfg)
        self.vocab = vocab
