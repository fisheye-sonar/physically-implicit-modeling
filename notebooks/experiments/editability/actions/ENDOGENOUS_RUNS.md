...
| `L3s0_ait_state` | L3 force+goal · strong · seed 0 · action-in-transition · **+ carried recurrent state** | 3 | strong + action-in-transition | 25000 it, `--carry-state --batched-sim` | 0 | **the fair GRU baseline**: both implementation flaws fixed (action reaches the state; the hidden state is no longer zeroed every 48 frames). The point of comparison for RSSM. |
...
**"weak" vs "strong"** is *only* these two configurations:
...
> **Every run in this table used `obs_noise_std = 0.05`. The repo standard is `0.2`** — used by *every* dataset
