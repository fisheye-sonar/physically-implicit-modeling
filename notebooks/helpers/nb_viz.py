...
__all__ = [
    "style_ax", "plot_color", "plot_waterfall_pair", "animate_3panel",
    "animate_gt_vs_predicted", "animate_ar_gt_vs_predicted",
...
# ── GT vs decoded animation ───────────────────────────────────────────────────


def animate_gt_vs_predicted(
    scene: Scene,
    obs_depth: np.ndarray,
    obs_id: np.ndarray,
    obs_intensity: np.ndarray,
    decoded_positions: np.ndarray,  # (T_dec, n_obj, 2) — typically T-1 from TF
    interval: int = 80,
    title: str = "",
    dark: bool = False,
) -> FuncAnimation:
    """2-panel animation: 2D scene with GT + decoded positions | actual waterfall.

    Ground-truth objects are drawn as solid filled circles with trails.
    Model-decoded positions are drawn as semi-transparent dashed-outline circles
    in the same colour, making alignment / drift immediately visible.

    Parameters
    ----------
    decoded_positions : (T_dec, n_obj, 2)
        Positions decoded from the model's hidden states.  T_dec is typically T-1
        when hidden states come from teacher-forcing (one h_t per input frame,
        so the final GT frame has no corresponding decoded position — the last
        decoded position is held for that frame).
    """
    bg      = _DARK_BG_HEX      if dark else _BG_HEX
    fe      = _DARK_FRUSTUM_EDGE if dark else _FRUSTUM_EDGE
    tc      = _DARK_TEXT_COLOR   if dark else _TEXT_COLOR
    ...
    T     = obs_intensity.shape[0]
    n_obj = scene.positions.shape[1]
    T_dec = decoded_positions.shape[0]  # T or T-1

    fig = plt.figure(figsize=(13, 5.5), facecolor=bg)
    fig.subplots_adjust(left=0.05, right=0.97, top=0.88, bottom=0.10, wspace=0.14)
    ax_w  = fig.add_subplot(1, 2, 1)
    ax_fa = fig.add_subplot(1, 2, 2)

    if title:
        fig.suptitle(title, color=tc, fontsize=11, y=0.97)

    for ax in (ax_w, ax_fa):
        style_ax(ax, dark=dark)
    ...
    ax_w.set_xlabel("x", color=tc, fontsize=10)
    ax_w.set_ylabel("depth  y", color=tc, fontsize=10)
    ax_w.set_title("2D scene  —  GT (solid)  /  decoded (dashed)", color=tc, fontsize=11, pad=6)
    ...
    # GT circles, trails, reflectivity labels
    gt_circles, gt_trails, refl_labels = [], [], []
    for i in range(n_obj):
        x0, y0 = scene.positions[0, i]
        circ = plt.Circle((x0, y0), scene.radii[i], color=scene.colors[i],
                           zorder=3, linewidth=1.2, edgecolor=ec, alpha=0.95)
        ax_w.add_patch(circ)
        gt_circles.append(circ)
        (trail,) = ax_w.plot([x0], [y0], color=scene.colors[i], linewidth=1.5,
                              alpha=trail_a, zorder=2, solid_capstyle="round")
        gt_trails.append(trail)
        lbl = ax_w.text(x0, y0, f"{scene.reflectivities[i]:.2f}",
                        ha="center", va="center", color="white",
                        fontsize=7, fontfamily="monospace", fontweight="bold", zorder=4)
        refl_labels.append(lbl)

    # Decoded (predicted) circles and dotted trails
    pred_circles, pred_trails = [], []
    for i in range(n_obj):
        x0d, y0d = decoded_positions[0, i]
        pred_circ = plt.Circle((x0d, y0d), scene.radii[i], color=scene.colors[i],
                                zorder=3, linewidth=2.0, edgecolor=ec,
                                alpha=0.40, linestyle="--")
        ax_w.add_patch(pred_circ)
        pred_circles.append(pred_circ)
        (pred_trail,) = ax_w.plot([x0d], [y0d], color=scene.colors[i], linewidth=1.0,
                                   alpha=trail_a * 0.55, zorder=2,
                                   solid_capstyle="round", linestyle=":")
        pred_trails.append(pred_trail)

    # Legend
    legend_handles = [
        Line2D([0], [0], color="gray", linewidth=3, alpha=0.95, label="GT"),
        Line2D([0], [0], color="gray", linewidth=3, alpha=0.40,
               linestyle="--", label="decoded"),
    ]
    ax_w.legend(handles=legend_handles, frameon=False, labelcolor=tc,
    ...
    img_actual = ax_fa.imshow(disp_actual, aspect="auto", origin="upper",
                               interpolation="nearest", extent=extent)
    ax_fa.set_xlabel("scan position", color=tc, fontsize=10)
    ax_fa.set_ylabel("frame", color=tc, fontsize=10)
    ax_fa.set_title("actual observation", color=tc, fontsize=11, pad=6)
    ax_fa.set_xlim(0, cfg.obs_res)
    ax_fa.set_ylim(T, 0)
    ...
    def update(f: int):
        # GT objects
        for i, (circ, trail, lbl) in enumerate(zip(gt_circles, gt_trails, refl_labels)):
            pos = scene.positions[f, i]
            circ.center = pos
            lbl.set_position(pos)
            t0 = _trail_t0(scene.positions[:, i], max(0, f - _TRAIL_LEN), f, cfg)
            trail.set_data(scene.positions[t0:f+1, i, 0],
                           scene.positions[t0:f+1, i, 1])

        # Decoded objects — hold last position if f >= T_dec
        f_dec = min(f, T_dec - 1)
        for i, (pred_circ, pred_trail) in enumerate(zip(pred_circles, pred_trails)):
            pos_d = decoded_positions[f_dec, i]
            pred_circ.center = pos_d
            t0_d = max(0, f_dec - _TRAIL_LEN)
            pred_trail.set_data(decoded_positions[t0_d:f_dec+1, i, 0],
                                decoded_positions[t0_d:f_dec+1, i, 1])
        ...
        return (gt_circles + gt_trails + refl_labels
                + pred_circles + pred_trails
                + [frame_text, img_actual, line_actual])

    return FuncAnimation(fig, update, frames=T, interval=interval, blit=True)


# ── AR GT vs decoded animation (3-panel) ─────────────────────────────────────


def animate_ar_gt_vs_predicted(
    scene: Scene,
    obs_depth: np.ndarray,
    obs_id: np.ndarray,
    obs_intensity: np.ndarray,
    pred_rollout: np.ndarray,       # (T - n_context, R) — AR predicted observations
    decoded_positions: np.ndarray,  # (T_dec, n_obj, 2) — from AR hidden states
    n_context: int,
    interval: int = 80,
    title: str = "",
    dark: bool = False,
) -> FuncAnimation:
    """3-panel animation: 2D scene (GT + decoded) | actual waterfall | predicted waterfall.

    Like animate_gt_vs_predicted but uses autoregressive rollout from n_context, and
    adds a predicted waterfall panel on the right so observation prediction quality
    is visible alongside the 2D position decoding.

    Parameters
    ----------
    pred_rollout : (T - n_context, R)
        Autoregressive predicted observations starting from frame n_context.
    decoded_positions : (T_dec, n_obj, 2)
        Positions decoded from the model's hidden states (context + AR rollout).
        Typically shape (T, n_obj, 2) from run_autoregressive.
    n_context : int
        Number of warm-up frames (shown dimmed in the predicted waterfall).
    """
    bg      = _DARK_BG_HEX      if dark else _BG_HEX
    fe      = _DARK_FRUSTUM_EDGE if dark else _FRUSTUM_EDGE
    tc      = _DARK_TEXT_COLOR   if dark else _TEXT_COLOR
    ...
    T     = obs_intensity.shape[0]
    n_obj = scene.positions.shape[1]
    T_dec = decoded_positions.shape[0]
