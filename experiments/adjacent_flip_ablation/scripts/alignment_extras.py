...
from othello_alignment import mine_board, replay, search_cf  # noqa: E402
...
def recoloured_during(h, s, rules):
    ...
    b = OthelloBoardState(**rules); placed = None
    for mv in h:
        before = b.state[s // 8, s % 8]
        b.umpire(mv)
        after = b.state[s // 8, s % 8]
        if before == 0 and after != 0: placed = after
        elif before != 0 and after != before: return True
    return False


def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--run", required=True); ap.add_argument("--n", type=int, default=900)
    ap.add_argument("--min-mass", type=float, default=0.99); a = ap.parse_args()
    ...
    with torch.no_grad():
        rs = model.residual_stack(torch.from_numpy(data.tokens[19000:19400]).to(DEV)); m = torch.from_numpy(data.mask[19000:19400]).to(DEV)
        cov_z = {p: torch.cov(zspace(lin[p], rs[p][m]).T) for p in range(NP)}
    cases = pickle.load(open(cases_path(inst), "rb")); rng = np.random.default_rng(0); sel = rng.permutation(len(cases))[: a.n]
    raw = []
    for i in sel:
        c = cases[i]; h = [int(x) for x in c["history"]]; s = int(c["pos_int"])
        best, d, m0 = search_cf(h, s, rules)
        if best is None or d > 0: continue
        hh, m1 = best; raw.append({"h": h, "hh": hh, "s": s, "L": len(h), "cur": int(m0[s]), "tgt": int(m1[s])})
    def pred(hists):
    ...
    for i, f in enumerate(raw):
        ...
        if float(p_cf[i, lg].sum()) >= a.min_mass: found.append({**f, "cf_i": i})
    n = len(found); print(f"{a.run}: {len(raw)} exact boards, {n} model-normal", flush=True)
    for f in found: f["recoloured"] = recoloured_during(f["h"], f["s"], rules)
    rec = np.array([f["recoloured"] for f in found]); Ls = np.array([f["L"] for f in found])
    print(f"  prefix length: mean {Ls.mean():.1f} (min {Ls.min()}, max {Ls.max()}); tile s recoloured during the game in {int(rec.sum())}/{n}", flush=True)
    # ceiling + unedited on the same cases
    legal_pre = [sorted(replay(f["h"], rules).get_valid_moves()) for f in found]
    def legal_post(f):
        b = replay(f["h"], rules); b.state[f["s"] // 8, f["s"] % 8] *= -1; return sorted(b.get_valid_moves())
    legal_post = [legal_post(f) for f in found]
    p_un = pred([f["h"] for f in found]); p_cfc = p_cf[[f["cf_i"] for f in found]]
    card_un = move_scorecard(p_un, legal_pre, legal_post); card_cf = move_scorecard(p_cfc, legal_pre, legal_post)
    print(f"  ceiling (true counterfactual) EI {card_cf['edit_index_union']:+.3f} vs unedited {card_un['edit_index_union']:+.3f}", flush=True)
    ...
    def resid(hists):
        ...
        with torch.no_grad():
            for L, ids in by.items():
                ...
                for p in range(NP):
                    for j, i in enumerate(ids): outs[p][i] = r[p, j]
        return {p: torch.stack(v) for p, v in outs.items()}
    R = resid([f["h"] for f in found]); Rcf = resid([f["hh"] for f in found])
    gen_idx = [int(rng.choice([j for j, g in enumerate(found) if g["L"] == f["L"] and j != i] or [i])) for i, f in enumerate(found)]
    g = torch.Generator(device="cpu").manual_seed(0)
    out = {"run": a.run, "instance": inst, "n_cases": n, "n_exact_boards": len(raw), "prefix_len_mean": float(Ls.mean()),
           "n_recoloured": int(rec.sum()), "ceiling_edit_index": card_cf["edit_index_union"], "unedited_edit_index": card_un["edit_index_union"],
           "ceiling_recoloured": None, "ceiling_lookup": None, "layers": {}}
    if rec.any() and (~rec).any():
        out["ceiling_recoloured"] = move_scorecard(p_cfc[rec], [x for x, r in zip(legal_pre, rec) if r], [x for x, r in zip(legal_post, rec) if r])["edit_index_union"]
        out["ceiling_lookup"] = move_scorecard(p_cfc[~rec], [x for x, r in zip(legal_pre, rec) if not r], [x for x, r in zip(legal_post, rec) if not r])["edit_index_union"]
    def cos2(A, B):  # rowwise
        return ((A * B).sum(1) ** 2 / ((A.norm(dim=1) ** 2) * (B.norm(dim=1) ** 2)).clamp_min(1e-12))
    for p in range(NP):
        probe = lin[p]; W = probe.net.weight.detach(); P = haufe_patterns(W, cov_z[p])
        z, zcf = zspace(probe, R[p]), zspace(probe, Rcf[p]); dz = zcf - z
        dgen = zspace(probe, R[p][torch.tensor(gen_idx, device=DEV)]) - z
        rows_i = torch.tensor([[f["s"] * N_CLASSES + c for c in range(N_CLASSES)] for f in found], device=DEV)
        d_raw = torch.stack([W[f["s"] * N_CLASSES + f["tgt"]] - W[f["s"] * N_CLASSES + f["cur"]] for f in found])
        d_hf = torch.stack([P[f["s"] * N_CLASSES + f["tgt"]] - P[f["s"] * N_CLASSES + f["cur"]] for f in found])
        ...
        fr_rows = torch.stack([frac_in(dz[i:i + 1], orth(W[rows_i[i]]))[0] for i in range(n)])
        fr_hf = torch.stack([frac_in(dz[i:i + 1], orth(P[rows_i[i]]))[0] for i in range(n)])
        fr_gen = torch.stack([frac_in(dgen[i:i + 1], orth(W[rows_i[i]]))[0] for i in range(n)])
        fr_gen_hf = torch.stack([frac_in(dgen[i:i + 1], orth(P[rows_i[i]]))[0] for i in range(n)])
        r_t = torch.from_numpy(rec).to(DEV)
        def split(v): return {"all": float(v.mean()), "recoloured": float(v[r_t].mean()) if r_t.any() else None, "lookup": float(v[~r_t].mean()) if (~r_t).any() else None}
        L = {"nd_dir_cos2_raw": split(c_raw), "nd_dir_cos2_haufe": split(c_hf), "nd_dir_cos2_random": float(c_rnd.mean()), "nd_dir_cos2_generic_raw": float(c_gen_raw.mean()),
             "rows_frac": split(fr_rows), "haufe_frac": split(fr_hf), "generic_rows_frac": float(fr_gen.mean()), "generic_haufe_frac": float(fr_gen_hf.mean()),
             "delta_norm_over_h_norm": float((dz.norm(dim=1) / z.norm(dim=1)).mean())}
        out["layers"][str(p)] = L
        print(f"pt {p}: ND-dir cos² raw {L['nd_dir_cos2_raw']['all']:.3f} (recol {L['nd_dir_cos2_raw']['recoloured'] or float('nan'):.3f} / lookup {L['nd_dir_cos2_raw']['lookup'] or float('nan'):.3f})"
              ...
              f" | haufe {L['haufe_frac']['all']:.3f} genH {L['generic_haufe_frac']:.3f}", flush=True)
    (EXP / "scores").mkdir(exist_ok=True)
    (EXP / "scores" / f"alignment_extras_{Path(a.run).name}.json").write_text(json.dumps(out, indent=1))
