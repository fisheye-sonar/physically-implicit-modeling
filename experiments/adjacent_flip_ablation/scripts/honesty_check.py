...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--run", required=True); ap.add_argument("--n", type=int, default=900); a = ap.parse_args()
    ...
    for i in rng.permutation(len(cases))[: a.n]:
        ...
        if bst is not None and d == 0: raw.append({"h": h, "hh": bst[0], "s": s, "ori": c["ori_color"]})
    p_cf = pred([f["hh"] for f in raw])
    keep = [i for i, f in enumerate(raw) if float(p_cf[i, sorted(replay(f["hh"], rules).get_valid_moves())].sum()) >= 0.99]
    found = [raw[i] for i in keep]; p_cf = p_cf[keep]
    ...
    c_un = move_scorecard(pred([f["h"] for f in found]), bench.legal_pre, bench.legal_post)
    c_cf = move_scorecard(p_cf, bench.legal_pre, bench.legal_post)
    out = {"run": a.run, "instance": inst, "n_cases": len(found), "unedited": c_un["edit_index_union"], "ceiling_true_counterfactual": c_cf["edit_index_union"],
           "cf_mass_on_legal_post": float(np.mean([p_cf[i, bench.legal_post[i]].sum() for i in range(len(found))])), "editors": {}}
    ...
    for ed, mode in (("PI", "pinv"), ("ND", "add_sub")):
        ...
        out["editors"][ed] = {"point": b["point"], "alpha": b["alpha"], "edit_index_union": card["edit_index_union"],
                              "mass_on_legal_post": float(np.mean([pr[i, bench.legal_post[i]].sum() for i in range(len(found))])),
    ...
    (EXP / "scores" / f"honesty_check_{Path(a.run).name}.json").write_text(json.dumps(out, indent=1))
