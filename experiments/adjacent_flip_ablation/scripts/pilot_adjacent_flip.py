...
never move again under adjacency).
...
def main():
    ...
    ap.add_argument("--instance", default="oth-adjacent-flip")
    ...
    legal_by_move = {}
    log_legal = []
    n_flipped, n_moves, n_wipeout = 0, 0, 0
    for g in games:
        ...
        for t, mv in enumerate(g):
            ...
            n_moves += 1
            if (b.state > 0).sum() == 0 or (b.state < 0).sum() == 0:
                wiped = True
        n_wipeout += int(wiped)
    ...
    rep = {
        ...
        "flips_per_game": n_flipped / a.n, "flips_per_move": n_flipped / n_moves,
        "frac_games_one_side_wiped_out": n_wipeout / a.n,
    ...
    (EXP / "scores" / "pilot_adjacent_flip.json").write_text(json.dumps(rep, indent=1))
    print(json.dumps(rep, indent=1))
    ok = rep["recolour"]["frac_changed"] >= a.min_change and rep["colour_eq_parity_frac"] < 0.9 \
        and rep["distinct_terminal_boards"] > 0.5 * a.n and rep["flips_per_game"] > 0
    print(f"\nGATE {'PASS' if ok else 'FAIL'}: recolourings changing the legal set "
          ...
          f"flips/game {rep['flips_per_game']:.1f} (need > 0); length {rep['length']['mean']:.1f}; "
          f"one side wiped out {rep['frac_games_one_side_wiped_out']:.1%}")
