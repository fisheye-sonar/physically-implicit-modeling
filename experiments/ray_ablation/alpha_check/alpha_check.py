...
re-runs the identical canonical arm (`arms.pinv_arm`, z-space, cached LIN probes — a
...
def main() -> None:
    ...
    for basis in ("cartesian", "frustum"):
        ...
        lin = dwa.fit_probes(model, target="full", n_seq=30_000, family="linear",
                             basis_name=basis, data_dir=INST / "probe",
                             cache_dir=RUN / "probes", log=print)     # cache HIT expected
        u = dwa.unsteered(model, b)
        rows = []
        for dims in ("pos", "all"):
            for r in dwa.pinv_arm(model, b, lin, ALPHAS, space="zspace", dims=dims):
