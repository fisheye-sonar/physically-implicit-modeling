...
lin = dwa.fit_probes(m, target="full", n_seq=N, family="linear", basis_name="frustum",
                     data_dir=root / "probe", cache_dir=CD, log=None)
mlp = dwa.fit_probes(m, target="full", n_seq=N, family="mlp", basis_name="frustum",
                     data_dir=root / "probe", cache_dir=CD, log=None)
...
arms = []
for dims in ("pos", "all"):
    arms += dwa.pinv_arm(m, b, lin, (1.0, 20.0), dims=dims)
    arms += dwa.nanda_arm(m, b, lin[1][0], 1, (0.5,), dims=dims)
    arms += dwa.grad_steer_arm(m, b, mlp, (0,), (0.1,), n_steps=5, dims=dims)
for r in arms:
...
_, st = dwa.observation_probes(target="full", n_seq=N, family="linear", basis_name="frustum",
...
f = dwa.fit_probes(rand, target="full", n_seq=N, family="linear", basis_name="frustum",
                   data_dir=root / "probe", cache_dir=CD, log=None)
print(f"random-init floor (LIN, n=500): {max(v[1]['r2'] for v in f.values()):+.3f}", flush=True)
roll = dwa.free_rollout(m, b.obs[:4], 20, 5)
