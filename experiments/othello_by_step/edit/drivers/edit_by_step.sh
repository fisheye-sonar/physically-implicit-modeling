...
set -euo pipefail
cd "$(dirname "$0")/../../.."
mkdir -p logs/othello_by_step/edit
EDITORS="${EDITORS:-PI ND}"
CASES="${CASES:-experiments/othello_by_step/edit/cases/synth_seed0_n256.pkl}"
systemd-run --user --unit=oth_edit_step -p MemoryMax=16G --collect \
  ...
  /usr/bin/bash -c '
    S=experiments/othello_by_step/edit/scripts; L=logs/othello_by_step/edit/edit_by_step.log
    {
