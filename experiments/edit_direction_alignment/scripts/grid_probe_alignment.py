...
for "Othello reads a classification, discworld a regression" (experiments/grid_target_control).
...
sys.path.insert(0, str(REPO / "experiments/grid_target_control/scripts"))
from common import frac_in, haufe_patterns, orth, zspace  # noqa: E402
from grid import N_CLASSES, cell_of  # noqa: E402
...
GRID = REPO / "experiments/grid_target_control"
run_dir = REPO / RUN
...
j = eobj[keep]
cell_real = cell_of(pos[keep, EF - 1, :][np.arange(len(keep)), j], sim)
cell_cf = cell_of(cf_pos[keep, EF - 1, :][np.arange(len(keep)), j], sim)
...
store = ProbeCache(GRID / "probes")
gp = json.loads((GRID / "scores/grid_probes.json").read_text())
grid_lin = {}
for ell, blk in gp["points"].items():
    c = blk["linear"]["cache"]
    grid_lin[int(ell)] = store.load(c["file"], c["prov"], device=DEV)[0]
print(f"grid probes loaded for points {sorted(grid_lin)} (skill "
      f"{min(gp['points'][str(p)]['linear']['skill'] for p in grid_lin):.3f}–"
      f"{max(gp['points'][str(p)]['linear']['skill'] for p in grid_lin):.3f})", flush=True)
