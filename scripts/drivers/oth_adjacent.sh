#!/usr/bin/env bash
# ── L-oth-adjacent-20m: Othello Transformer-L on the ADJACENCY-PLACEMENT variant (2026-09-08) ──
#
# Identical to flip_ablation/L-oth-noflip-20m (their GPT, 8 x 512, 61-way head, 20M games,
# 780k steps, batch 256, lr 1e-3 constant, wd 1e-4, clip 1, seed 0; no recolouring) — the ONLY
# change is the placement rule: a move is legal iff the square touches one of the mover's own
# discs (8-neighbourhood) instead of enclosing opponent discs. Colour is therefore causally
# relevant WITHOUT the enclosure geometry (oth-noflip collapsed to a colour-free game).
# Same passes, game end, sampling and index law (datasets/othello/oth-adjacent/instance.json).
#
...
# Launch:
#   systemd-run --user --unit=oth_adjacent -p MemoryMax=40G --collect --working-directory=$PWD \
#       /usr/bin/bash -c 'bash scripts/drivers/oth_adjacent.sh > logs/adjacency_ablation/L-oth-adjacent-20m/unit.log 2>&1'
set -u
cd "$(dirname "$0")/../.." || exit 1
ROOT=$PWD
...
TOPIC=adjacency_ablation
NAME=L-oth-adjacent-20m
INST=oth-adjacent
...
ping "PIM $NAME: chain started" "oth-adjacent: pilot gate -> corpus 20M (~70 min) -> cases -> train 780k (~20 h) -> score -> tables."

stage "P pilot gate (CPU) $INST"
"$PY" -u experiments/adjacency_ablation/scripts/pilot_adjacent.py > "$LOGS/p_pilot.log" 2>&1 \
