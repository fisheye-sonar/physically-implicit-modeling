...
if [ -f "$RUN_DIR/best_model.pt" ] && [ -f "$RUN_DIR/metrics.jsonl" ] \
  ...
  # park the old-budget scores BEFORE training (2026-09-23; was after): the trainer writes the new budget
  # into config.json the moment it starts, and for the hours of the extension the tables would otherwise pool
  # 390k scores under the 512k budget (seen 2026-09-20; the queue's dispatcher parks at launch too — idempotent)
...
"$PY" scripts/layout_checkpoint_replicate.py "$PARENT" "nearest:$STEPS" \
