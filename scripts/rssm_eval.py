"""Command-line RSSM evaluation script.

Runs all four evaluation criteria (or a subset) for a trained RSSM checkpoint
and saves figures + metrics to a timestamped output directory.

Usage:
    python scripts/rssm_eval.py \\
        --checkpoint runs/rssm_baseline/best_model.pt \\
        --test-h5    datasets/my_dataset/val.h5 \\
        --edits-h5   datasets/my_edits/dataset.h5 \\
        --output-dir outputs/eval_rssm_run1 \\
        --criteria 1 2 3 4 \\
        --device cuda

    # Run only predictive quality and recovery:
    python scripts/rssm_eval.py ... --criteria 1 2
"""
...
from pim.world_models.rssm.run_eval import (
    EvalConfig,
    plot_criterion1,
    plot_criterion2,
    plot_criterion3,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="RSSM model evaluation")
    ...
    p.add_argument("--edits-h5", required=True, dest="edits_h5", help="Edits dataset HDF5")
    ...
    p.add_argument("--rollout-n-context", default=20, type=int, dest="rollout_n_context")
    p.add_argument("--rollout-n-rollout", default=20, type=int, dest="rollout_n_rollout")
...
def main() -> None:
    ...
    if "c3" in results:
        all_figs.update(plot_criterion3(cfg, s, results["c1"], results["c3"]))
    if "c4" in results:
