...
the nearest validated val_loss, and stamped with a `replicate` block.
    python scripts/layout_checkpoint_replicate.py noise_ablation/L-dw-noiseless-20m 421875
    python scripts/layout_checkpoint_replicate.py ray_ablation/L-dw-8ray-20m nearest:390000

Every seed-replicate set's seed-0 member — so every ± in the main tables — goes through this script
(``scripts/drivers/replicate.sh`` stage B). In ``scripts/`` since 2026-09-19 (was
``experiments/seed_variance/scripts/``, where a forwarder remains for the jobs already queued).
"""
import json
import shutil
import sys
