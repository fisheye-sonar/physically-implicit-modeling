...
Writes datasets/discworld/<instance>/tokens/ (train.i16, probe/val/test/edits.npy,
vocab.npz, meta.json). Additive; safe to re-run (overwrites the tokens/ files only).
...
def main() -> None:
    ...
    a = p.parse_args()
    tokenize_instance(_REPO / "datasets" / "discworld" / a.instance, chunk=a.chunk)
