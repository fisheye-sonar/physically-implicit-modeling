...
## 🔄 dw-pair chain on the WSL remote — 2026-09-15 evening (unit `dw_pair`, `scripts/drivers/dw_pair.sh`, logs `logs/pair_ablation/dw_pair/` on wsl-sevan)

Sevan: dw-noiseless with the two discs as a rigid pair (centre distance 2.0, same velocity; `SimConfig.pair_separation`, one branch in `sim.py`, `--pair-separation` on the generator, test in `tests/test_sim.py`). Single-object teleport bench kept on purpose (the edits should fail), targets unchanged, regression in frustum + cartesian with both floor sets, categorical = appearance-fac only. Registered in `bigcorpus.INSTANCES` (seeds train 300e9 / eval 325e9 / probe 1100e9 / probe_large 1110e9). Stages B generate (~2 h; 410 GB corpus, 761 GB free there) → C train (~12 h) → D master_eval + tables → E appearance-fac probes + floors + score (~3–4 h). ETA all done ≈ 2026-09-16 afternoon. Afterwards: pull run / probes / scores / logs and the instance's probe, eval and edits splits (NOT the corpus), record. Branch `dw_pair` (on top of `dw_16ray`).
