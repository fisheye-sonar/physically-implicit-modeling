...
## ✅ dw-pair chain on the WSL remote — 2026-09-15 20:42 → 09-16 12:20 PT, unit `dw_pair` — DONE (`scripts/drivers/dw_pair.sh`, logs `logs/pair_ablation/dw_pair/` + `logs/dw_pair_fac/`, pulled to the lab)

Sevan's toggle: dw-noiseless with the two discs as a rigid pair (centre distance 2.0, shared velocity; `SimConfig.pair_separation`), scored on the UNCHANGED single-object teleport bench because the broken-constraint target is the point. Generate 64 min (410 GB corpus), train 666 min (best val 7.80e-4 vs dw-noiseless 1.06e-3), canonical scoring in both bases 61 min, appearance-fac probes + floors + rescore 118 min. **Result:** the edits degrade but do not fail — `appearance-fac` ND +0.490 / 0.91 and GS +0.294 / 0.86 against dw-noiseless's +0.612 / 0.80 and +0.326 / 0.71, while decodability moves the other way (0.433 → 0.448 LIN; per-dim velocity skill 0.40–0.70 → 0.76–0.93, the shared velocity read from two discs). Regression rows inert in both bases as on every 128-ray instance. ⚠ The run cannot separate "less writable state" from "off-manifold target"; the control is a PAIR-teleport bench (both discs by the same vector), an option in the edit generator, not a new run. IM / IM-NN filled in 2026-09-16 14:12 (the editors were missing only because the remote was on a branch without `pim/editors/inverse.py`; the lab branch was pushed and checked out there): frustum IM +0.471 / 0.60, IM-NN +0.230 / 0.91 against dw-noiseless's +0.598 / 0.34 and +0.397 / 0.65, with g R² 0.753 vs 0.750 — every editor that lands elsewhere lands here too, each weaker and each damaging the frame more. Sevan's bet (IM fails) lost; my nominated call (IM − IM-NN gap widens past 0.25) also failed — it widened 0.202 → 0.241 in frustum and NARROWED in cartesian, so the gap is not a discriminator. Written up in `findings/pair-ablation.md`; REGISTRY instance + run rows. Branch `dw_pair` (on top of `dw_16ray`) — PR pending for both.

## ✅ Fixed grids on the ray axis — 2026-09-15 09:33 → 17:0x PT on the WSL remote, unit `grid_targets_5_16` — DONE

Sevan: separate target granularity from observation resolution. `grid-8x4` + `grid-16x8` fitted (probes, floors, rescore) on L-dw-5ray-20m and L-dw-16ray-20m; 8-ray and 128-ray already had them. Result: under a fixed grid 8 / 16 / 128 rays are level on PI and GS and ND is flat; only 5-ray keeps an edge, and only on GS (+0.45 / +0.50 vs ~+0.31). Most of the observation-exact ray trend was the target's own granularity; the radius confound at 128 rays is moot on the fixed grids (128-ray radius-1.0 run held). Written up in `findings/probe-target-type.md` §Fixed-resolution grids across the ray axis; REGISTRY rows. Hiccups: the 5-ray grid-8x4 scoring kernel died once under a 40 GB cap (no trace; passed at 44 GB); a stale FAILED line tripped my watcher once. Probes/scores/floors/logs pulled to the lab; 5-ray and 16-ray lack the IM blocks the score_im chain added elsewhere. Branch `dw_16ray` (PR pending).

## ✅ dw-16ray chain on the WSL remote — 2026-09-14 11:15 → 09-15 03:54 PT, unit `dw_16ray` — DONE (`scripts/drivers/dw_16ray.sh`, logs `logs/ray_ablation/dw_16ray/` + `logs/dw_16ray_fac/`, pulled to the lab)

Sevan: the ray axis upward — dw-8ray with 16 usable rays (18 cast, wall rays dropped, radius 1.0), otherwise identical. Generate 2 h 47 (16 workers; 51 GB corpus), train 11 h 08 on the 4090 (best val 0.003968 at 765k), canonical score 14 min, appearance-fac probes + floors + rescore 1 h 51. **Result** (eval 2026-09-12.2): regression inert (PI +0.245 / fid 1.45); `appearance-fac` PI +0.10 / 1.39, ND +0.42 / 0.90, GS +0.28 / 0.85 — between 8-ray (+0.38 / +0.49 / +0.46) and 128-ray (+0.01 / +0.61 / +0.33): coarser observation → more writable state holds through 16 rays (PI and GS monotone along 5 → 8 → 16 → 128; ND flat-to-down until 128). Written up in `findings/probe-target-type.md` §The ray axis upward + `ray-ablation.md` addendum; REGISTRY instance + run rows. Two hiccups, both fixed in the driver: (1) the 5-ray template predated the 2026-09-12 edit-selection rule — `make_edit_selection.py` run by hand at 14:08, step added to the driver; (2) stage D failed once at 01:46 because master_eval rescans every run dir and hit the remote's old-layout Othello runs — they were parked under `runs/_synced_to_lab/` (ledgered in the remote's `runs/MOVES.md`; the lab copies are authoritative) and the chain relaunched with `--resume` on the training stage (a no-op). Synced to the lab: run dir (2.6 GB), `_baselines/dw-16ray`, logs, `datasets/discworld/dw-16ray/{probe,eval,edits,layout.json,instance.json}` (786 MB) — not `train/`. Joint-cell `appearance` block done 05:45 (unit `dw_16ray_app`): PI +0.23 / 1.32, ND +0.27 / 1.07, GS +0.44 / 0.57 — below 8-ray on every editor; probes + scores re-pulled. Nothing in flight on either GPU. Branch `dw_16ray` (from main) holds the instance registration, driver, scorer row and these records — merge via PR.
