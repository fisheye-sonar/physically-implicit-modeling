...
| `dw-16ray` | discworld | `datasets/discworld/dw-16ray/instance.json` | `dw-8ray` with the observation raised to **16 usable rays** (18 cast, wall rays dropped; radius 1.0) — the ray-count axis upward at one radius: dw-5ray (5) → dw-8ray (8) → dw-16ray (16); dw-noiseless is 128 rays at radius 0.5. Seeds train 270e9, eval 295.2e9, edits 295.3e9, probe 1080e9, probe_large 1090e9 (`bigcorpus.INSTANCES`). Generated 2026-09-14 on the WSL remote by `scripts/drivers/dw_16ray.sh` (unit `dw_16ray`), layout v2 from birth. Purpose (Sevan): does the coarse-observation editability gain persist, grow or reverse at 16 rays? |
| `dw-pair` | discworld | `datasets/discworld/dw-pair/instance.json` | `dw-noiseless` with the two discs as a **rigid pair**: object 1 at centre distance **2.0** from object 0 (random orientation), same velocity (`SimConfig.pair_separation`, `sim.py`; open boundary only, ≥ the collision spacing). The state has 2 free position dims + an orientation instead of 4. Single-object teleport bench KEPT (the edits are expected to fail — Sevan's point); targets unchanged; regression scored in frustum AND cartesian with both floor sets. Seeds train 300e9, eval 325.2e9, edits 325.3e9, probe 1100e9, probe_large 1110e9. Generated 2026-09-15 on the WSL remote by `scripts/drivers/dw_pair.sh` (unit `dw_pair`), layout v2 from birth. |
...
| `ray_ablation/L-dw-5ray-20m` | Transformer-L (regression) | dw-5ray | 780k steps, matched recipe — the quantisation push (launched 2026-09-10 night, unit `dw_5ray`, `scripts/drivers/dw_5ray.sh`; `Linear(5, 512)` in/out). Scored canonically + `appearance-fac` (stage E). Best val MSE 0.00694 at 450k (473 min). Canonical frustum: skill 0.94 / 0.97 (rand-init 0.95 / 0.96), PI +0.30 / 1.12, GS −0.11 — inert. **`appearance-fac` (9 + 4 classes, 52 logits): skill 0.93 / 0.93, PI +0.49 / 0.68, ND +0.56 / 0.90, GS +0.58 / 0.47 — above dw-8ray on every editor (+0.41 / +0.50 / +0.46); Sevan's prediction held.** Joint-cell `appearance` (14 × 3): skill 0.88 / 0.89, PI +0.54 / 0.78, ND +0.47 / 0.85, **GS +0.64 / 0.40** (8-ray: +0.43 / +0.43 / +0.61) — the bump replicates; highest discworld GS. No filtered selection file yet (canonical rows use the first 192 cases). `findings/probe-target-type.md`. Fixed grids (2026-09-15, remote unit `grid_targets_5_16`): `grid-8x4` skill 0.32 / 0.36, PI +0.34 / 1.02, ND +0.32 / 1.08, GS +0.45 / 0.60; `grid-16x8` skill 0.05 / 0.09, PI +0.16 / 1.58, ND +0.30 / 1.11, GS +0.50 / 0.66 — GS keeps its edge on a target the model can barely read. |
| `ray_ablation/L-dw-16ray-20m` | Transformer-L (regression) | dw-16ray | 780k steps, matched recipe (2026-09-14 14:02 → 15 01:31 on the WSL remote, unit `dw_16ray`, 11 h 08 on the 4090; generated there 11:15 → 14:02). Best val MSE 0.003968 at 765k. Scored on the remote at eval 2026-09-12.2 and pulled to the lab with `runs/_baselines/dw-16ray/`, logs, and the instance's probe / eval / edits splits (NOT the 51 GB corpus). Frustum: skill LIN 0.954 / MLP 0.991, unedited −0.910, PI +0.245 / fid 1.45, ND −0.203 / 0.90, GS −0.133 / 0.84. `appearance-fac` (34 classes/object, 136 logits; rand-init floor 0.81 / 0.89, obs-right 0.36 / 0.89): skill 0.879 / 0.918, PI +0.100 / 1.39 (pt 1, α 100), ND +0.423 / 0.90 (pt 2, α 8), GS +0.280 / 0.85 (pt 0, α 1.5) — between 8-ray and 128-ray on every editor. `appearance` (joint cell; rand-init floor 0.81 / 0.84, obs-right 0.55 / 0.85): skill 0.850 / 0.873, PI +0.233 / 1.32 (pt 3, α 60), ND +0.270 / 1.07 (pt 3, α 12), GS +0.435 / 0.57 (pt 0, α 0.7) — every editor below 8-ray's joint-cell row (unit `dw_16ray_app`, 1 h 50). Fixed grids (2026-09-15, remote unit `grid_targets_5_16`): `grid-8x4` skill 0.65 / 0.73, PI +0.32 / 0.87, ND +0.29 / 1.17, GS +0.31 / 0.71; `grid-16x8` skill 0.33 / 0.43, PI +0.15 / 1.38, ND +0.27 / 1.18, GS +0.32 / 0.69 — level with 8-ray and 128-ray once the target is fixed. `findings/probe-target-type.md` §The ray axis upward; `ray-ablation.md` addendum. |
| `pair_ablation/L-dw-pair-20m` | Transformer-L (regression) | dw-pair | 780k steps, matched recipe (2026-09-15 21:46 → 09-16 09:21 on the WSL remote, unit `dw_pair`, 666 min on the 4090). Best val MSE 7.80e-4 at 765k (dw-noiseless 1.06e-3 — the constrained world is easier to predict). Scored at eval 2026-09-12.2 in BOTH bases with both floor sets + `appearance-fac`; pulled to the lab without the 410 GB corpus. frustum: skill 0.944 / 0.998, unedited −0.951, PI +0.243 / 1.63, ND −0.114 / 1.87, GS −0.093 / 1.02. cartesian: 0.922 / 0.994, PI +0.229 / 1.86, ND −0.113 / 1.72, GS −0.109 / 1.04. `appearance-fac` (262 classes; floors 0.31 / 0.62, obs-right 0.12 / 0.53): skill 0.448 / 0.798, **ND +0.490 / 0.91**, GS +0.294 / 0.86, PI −0.022 / 1.72 — vs dw-noiseless's ND +0.612 / 0.80 and GS +0.326 / 0.71: the edits degrade but do not fail. IM / IM-NN added 2026-09-16 on the remote (unit `score_im_pair`, branch `sweeps_and_blates`): frustum **IM +0.471 / 0.60**, IM-NN +0.230 / 0.91 (dw-noiseless +0.598 / 0.34 and +0.397 / 0.65); g R² 0.753 vs 0.750, so the drop is not a bad-map artefact. `findings/pair-ablation.md`. |
