...
| `ray_ablation/L-dw-16ray-20m` | Transformer-L (regression) | dw-16ray | IN FLIGHT (2026-09-14 → on the WSL remote, unit `dw_16ray`, driver `scripts/drivers/dw_16ray.sh`): 780k steps, matched recipe — the ray axis upward from dw-8ray. Scored on the remote at the current eval version (same code as the lab), then pulled back with the instance's probe / eval / edits splits (not the 51 GB corpus). Extra targets: `appearance-fac`, `appearance` (master_eval `dw_extra_targets`). |
