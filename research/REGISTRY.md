...
| `oth-adjacent-flip` | othello | `datasets/othello/oth-adjacent-flip/instance.json` | `oth-adjacent` with RECOLOURING back on (2026-09-09): a move is legal iff the empty square touches one of the mover's own discs (8-neighbourhood), AND the placed disc recolours the discs it encloses along any line, exactly as in Othello (`OthelloBoardState(flip=True, placement='adjacent')`); same passes / game end / uniform sampling / index law / vocabulary. Colour is used by legality AND rewritten by the dynamics, so it is no longer decodable from the input — the 'used and not input-decodable' cell oth-adjacent could not reach. Pilot PASSED 2026-09-09 (70% of recolourings change the legal set, 16 flips/game, Bayes CE 2.296); corpus 20M + splits + 1001 synthesised cases built 2026-09-10 on the WSL remote; edits = 1001 synthesised cases (`scripts/make_othello_edits.py`). `experiments/adjacent_flip_ablation/`. |
| `dw-5ray` | discworld | `datasets/discworld/dw-5ray/instance.json` | `dw-8ray` with the observation cut to **5 usable rays** (7 cast, wall rays dropped; radius 1.0 — the floor for 5 rays: at 4 kept rays a reachable disc can light no ray). Appearance partition **14 cells** (run lengths 1–4, 9 centres; factorised 9 + 4 = 13 classes/object) vs 30 on dw-8ray. Seeds train 160e9, eval 185.2e9, edits 185.3e9, probe 1020e9, probe_large 1030e9 (`bigcorpus.INSTANCES`). Generated 2026-09-10 night by `scripts/drivers/dw_5ray.sh` (unit `dw_5ray`), layout v2 from birth. Purpose: does quantisation past 8 rays raise editability under the factorised categorical target? |
| `dw-128ray` | discworld | `datasets/discworld/dw-128ray/instance.json` | `dw-8ray` with the observation raised to **128 usable rays** (130 cast, wall rays dropped; radius 1.0; max-edit-attempts 2000) — the ray-count axis closed at the top at one radius: dw-5ray (5) → dw-8ray (8) → dw-16ray (16) → dw-128ray (128). NOT dw-noiseless (128 cast, no drop, radius 0.5): against it this instance separates ray count from disc size. Seeds train 300e9, eval 325.2e9, edits 325.3e9, probe 1100e9, probe_large 1110e9 (`bigcorpus._RAYS128_RANGES`, forbidden to every other instance). Built 2026-09-15 night on the lab box (`scripts/drivers/dw_128ray.sh`). |
| `dw-smooth` | discworld | `datasets/discworld/dw-smooth/instance.json` | `dw-noiseless` (128 rays, radius 0.5, no noise, 20M) with the disc's radial profile changed from a flat plateau to the **power dome**: intensity = reflectivity × (1 − (perp/r)²)² (`SimConfig.soft_shading="power"`, `soft_profile_power=2.0`, `soft_edge=0` — the dome itself reaches zero at the silhouette; both renderer backends, pinned by `tests/test_soft_render.py`; `zone_editability.sim_config_from` passes the knob so the bench's reference renders match). The anti-aliasing instance: every covered ray's value varies continuously with sub-ray position. Covered-ray geometry unchanged → appearance partition and its factorisation are dw-noiseless's (2,889 cells; 233 + 29 classes). Profile H of `experiments/antialias_pilot`, Sevan's pick 2026-09-12. Seeds train 200e9, eval 225.2e9, edits 225.3e9, probe 1040e9, probe_large 1050e9 (`bigcorpus._SMOOTH_RANGES`, forbidden to every other instance and vice versa). Generated 2026-09-12 night by `scripts/drivers/dw_smooth_gen.sh` (unit `dw_smooth_gen`), layout v2 from birth; edit selection = the first 1000 cases with ≥ 2 differing rays. Purpose: does a continuous observation restore the geometric read-out's editability that dw-noiseless lacks? |
| `dw-8ray-obs5` | discworld | `datasets/discworld/dw-8ray-obs5/instance.json` | `dw-8ray` seen by **five observers** on a ring about the frustum's depth midpoint (0, 7.5) — observer k = the canonical observer rotated by 2πk/5 — with the discs confined to the **circle of radius 4.5** every observer's near/far planes are tangent to (`SimConfig.n_observers=5`, `region="circle"`; `pim/environments/discworld/observers.py`, 2026-09-13; defaults bit-identical, `tests/test_observers.py`). Observation = 5 × 8 kept rays = **40 values**, observer-major. Every arena position lights ≥ 1 kept ray in every view; a disc is still invisible to a given observer ~7 % of the time through occlusion by the other disc, never to all five. Read-outs: **Cartesian** regression basis (frustum is observer-0-relative) and the per-observer factorised appearance (5 × (15 + 5) classes per disc). Seeds train 230e9, eval 255.2e9, edits 255.3e9, probe 1060e9, probe_large 1070e9 (`bigcorpus._OBS5_RANGES`). Generated 2026-09-13 evening by `scripts/drivers/dw_8ray_obs5.sh` (unit `dw_8ray_obs5`). Purpose: position more determined through partial-view integration; is the categorical editability of 8-ray about observation coarseness or about a small output? |
...
| **INVERSE MAP** (canonical 2026-09-15) | `pim/probes/inverse.py` — g: environment state → residual at a point, E[h ∣ state]; the MIRROR of MLP-128 (one hidden layer of 128, the probes' 200-epoch recipe, the same seeded 80/20 split by sequence), one g per residual point, cached like every probe (kind `inverse_map`, fingerprinted). Input: discworld — the FULL state (position + velocity of both discs) in the block's basis; Othello — the one-hot mine/theirs board (64 × 3). Beside it `RetrievalBank`: the k = 10 nearest training states' mean residual (standardised Euclidean / one-hot Hamming), never cached. Its held-out R² (`inverse_map.g_r2` per block) says how much of the residual the state explains — not a decodability number and never tabulated as one. | the instrument of the IM editor; measured 2026-09-14 across 12 runs (`findings/inverse-probe.md`) |
...
**Seed replicates** (2026-09-11). A re-training of a canonical run with another seed lives at `runs/<topic>/<parent>__seed<k>/` with a `replicate` block in `config.json` (`scripts/train.py --replicate-of <topic>/<parent>`; a checkpoint of the parent at the replicates' step budget via `experiments/seed_variance/scripts/layout_checkpoint_replicate.py`, `"checkpoint": true`). Scored like any run (a replicate INHERITS its parent's extra targets — `master_eval` `extra_targets_of`, 2026-09-14); **never a table row** — `pim.figures.tables.pool_replicates` pools the replicate set per (parent, basis) into the parent row's ± (SD, ddof 1), **at a matched training budget only (±10%) by default**, listing any budget it left out; `collect(pool_budgets=True)` (the `POOL_BUDGETS` knob at the top of both table notebooks) overrides and Table 5 then says 'mixed budgets'. One replicate end to end: `scripts/drivers/replicate.sh <topic/parent> <seed> <steps> [categorical targets]` (trains, lays out the parent's nearest checkpoint, fits the targets, scores). Probe-seed refits: `experiments/seed_variance/scripts/probe_seeds.py` (discworld) / `probe_seeds_othello.py`. Probe-seed spread (`fit_probes(seed=k)`, cached beside seed 0) → `runs/<run>/variance.json` (`experiments/seed_variance/scripts/probe_seeds.py`), Table 4b. First set: `L-dw-noiseless-20m__seed1`, `__seed2` (390k steps), `__seed0_s421875`.

**Bases** — since 2026-09-11 the canonical discworld regression basis is **frustum only**: cartesian is no longer scored for new runs (`master_eval` SETTINGS `dw_bases`) and `build_full_table` hides it (`HIDDEN_BASES`); the cartesian blocks and floors of every run scored before that date remain in their `scores.json` / `baselines.json` as a record (frustum settled 2026-09-01; the 2026-09-11 cut-off is `L-dw-5ray-20m`, the last run with a cartesian block); **2026-09-14 evening (Sevan): a cartesian block was ADDED to every discworld run as a paper-option record** — `scripts/drivers/score_cartesian.sh` runs master_eval with `PIM_DW_BASES=frustum,cartesian` (the SETTINGS hook defaults to frustum); frustum stays canonical and the tables still hide cartesian; verify table in `logs/score_cartesian/verify.txt`: cartesian PI a few hundredths below frustum on the 128-ray runs, 0.05–0.10 below on 8-ray / 5-ray, GS negative in both).
...
| **`grid-16x8`** | discworld | classification, 128 cells × 3 {empty, obj 0, obj 1} | `pim/environments/discworld/grid_target.py` — cells uniform in the frustum basis (u′, 1/y) over the reachable region; nearer object wins a shared cell; a same-cell teleport is a no-op and is dropped from its bench (`bench.grid_selection`) | `arms.GRID_PROBE_RECIPE`: `probe_250k` split, 200k seq, 50 epochs, streamed (`fit_probe_stream`); LIN is SGD-CE (no closed form) | `noise_ablation/L-dw-noiseless-20m` (probes fitted 2026-09-08 in `experiments/grid_target_control`, re-keyed into the run's `probes/` 2026-09-09; never fitted by the scorer — `require_cached`), and since 2026-09-10 both 8-ray runs as one rung of the resolution sweep (`GridTarget(nu, nd)` for any `grid-<nu>x<nd>`; noiseless sweep `grid-8x4`, `-32x16`, `-64x32`, `appearance-lat` = chain 4). Opt-in per run: `master_eval` SETTINGS `dw_extra_targets` |
| **`appearance`** (+ `-d2`, `-d3`, `-lat`) | discworld | classification, cells = the RUNS of rays a disc lights (the observation-exact partition); dw-8ray: 30 cells × 3 = 90 logits; `-d<k>` splits each run into k depth bands (uniform in 1/y over the run's depth range), `-lat` merges runs by centre (15 cells) | `grid_target.AppearanceTarget` — `covered_rays` is the renderer's own ray–disc test (gated equal to `render_frame`); the realisable runs come from a deterministic dense sweep of the reachable region | `GRID_PROBE_RECIPE` (200k seq, 50 epochs, streamed), fitted by `scripts/fit_probes.py` | `ray_ablation/L-dw-8ray-20m`, `interface_ablation/L-dw-8ray-tok-20m` (overnight 2026-09-09→10). The **resolution sweep** on both: product grids `grid-4x2`, `-8x4`, `-16x8`, `-32x16` (512 cells: skill 0.01 / 0.04 yet GS +0.42 / 0.68 — editability under a product grid is insensitive to Probe Skill; `-64x32` stopped: LIN at the majority error) and the MISALIGNED 30-cell controls `grid-6x5`, `grid-10x3`; `appearance` is the unimodal peak (frame model GS +0.61 / 0.39 vs +0.31…+0.37 for every product grid; the 30-cell controls +0.34 — alignment, not count). Every teleport of the filtered 8-ray bench changes cell; two objects share a cell in 0.31% of frames |
...
| **`pos@<partition>`** (snapped regression; e.g. `pos@appearance`) | discworld | **regression**, 4 outputs — every object's position replaced by the CENTRE of its cell under the named partition, in the frustum basis (`grid_target.SnappedTarget`; `full@…` keeps the basis velocities alongside) | `CategoricalTarget.centroids` / `.snap`: the cell's centroid in (u, 1/y) under a uniform sweep of the reachable region, or its medoid where the centroid leaves a non-convex cell — always a realisable position inside the cell; `frustum_to_world` inverts the basis. Bench: the partition's cell-changing teleports (`grid_selection`), the regression branch otherwise (PI via `pinv_target`, GS via the MSE spec; ND computed, not reported — continuous target) | the canonical regression recipe (120k corpus, 30k sequences, 200 epochs), fitted INLINE by the scorer like `pos`/`full`; floors: the canonical set (four observation layouts + random-init), inline | `ray_ablation/L-dw-8ray-20m` (`pos@appearance`, 2026-09-10): skill 0.96 / 0.99, PI +0.34 / 0.92 (α 100), GS −0.13 / 0.84 — the REGRESSION row reproduced, not the categorical one (+0.43 / +0.61): the categorical gain is the read-out and its logit-swap edit, not the target's alignment. `findings/probe-target-type.md` |
| **`<partition>-fac`** (factorised categorical; e.g. `appearance-fac`) | discworld | classification, OBJECT-indexed: per object one softmax per FACTOR of the partition — appearance: (run centre, run length) = 15 + 5 classes on dw-8ray; a product grid: (lateral bin, depth bin) — N_OBJ × F tiles over one shared class axis (each factor's classes offset, so a class index means one thing and the pooled majority reads as on Othello) | `grid_target.FactorisedTarget`; labels `factor_labels`, edit `edit_moves` → `Bench.moves` {tile, old, new}; PI swaps old ↔ new at every moved tile (`swap_class_logits`), ND adds the summed row contrast (`arms.categorical_direction`), GS asks CE toward the new labels; bench = the partition's cell-changing cases (`selection_target`) | `GRID_PROBE_RECIPE` (250k corpus, 200k seq, 50 epochs, streamed), fitted by `scripts/drivers/probe_target_fit.sh` (model + both floors) | `ray_ablation/L-dw-8ray-20m` (`appearance-fac`, 2026-09-10): skill 0.94 / 0.94 (rand-init 0.92 / 0.94, obs-right 0.48 / 0.94), **PI +0.41 / 0.71, ND +0.50 / 0.86, GS +0.46 / 0.53** vs the joint cell's +0.43 / +0.43 / +0.61 — the categorical gain survives factorisation (GS loses the joint move). `noise_ablation/L-dw-noiseless-20m` (`appearance-fac`, 233 + 29 classes, 1,048 logits, 2026-09-10 evening): skill 0.43 / 0.79 (rand-init 0.30 / 0.59, obs-right 0.12 / 0.52 — training's margin), **ND +0.63 / 0.78 — Othello's level (+0.62)**, GS +0.35 / 0.68, PI +0.01 / 1.95 (ill-conditioned pseudo-inverse at 1,048 rows). `findings/probe-target-type.md` |
...
| **IM** (canonical 2026-09-15, Sevan) | `pim/editors/inverse.py` — the inverse-map OVERWRITE h′ = g(s_post) at the edit position: the residual replaced by the state-conditional mean of the TARGET state (discworld: the bench's pre-dynamics full state — `bench.full_state_pair`; Othello: the board with the case's tile flipped). Parameter-free (no α, no probe); scored at every residual point with the canonical scorecards + guard, on every block's bench (the write is a state write — one g per basis serves the regression AND the categorical blocks of a run). ⚠ Sees the WHOLE target state, more than PI writes through the probe rows, and writes it into a state no game / episode produced. `IM-NN` = the retrieval form (`RetrievalBank`, k 10), computed into `scores.json` beside it, NOT tabulated. The α-swept `inverse_delta` (h + α(g(s_post) − g(s_pre))) is kept non-default: ≤ +0.15 over the overwrite at its best α, never a different verdict (`findings/inverse-probe.md`). Tables show **PI, GS, IM** in that order (`pim.figures.tables.EDITORS`); ND is computed and kept in `scores.json`, no longer tabulated by default (2026-09-15). |
...
| `noise_ablation/L-dw-noiseless-20m` | Transformer-L (regression) | dw-noiseless | 780k steps, matched recipe — the noise ablation (2026-08-31). **`appearance-fac` (2026-09-10): ND +0.63 / 0.78 — the first discworld edit at Othello's level; skill 0.43 / 0.79 above both floors.** The first run carrying the **`grid-16x8` probe target** (2026-09-09): skill LIN 0.43 / MLP 0.71, unedited −0.93, PI +0.13 / fid 1.58, **ND +0.37 / 0.91**, **GS +0.29 / 0.87** (one-frame, reverting); `figures/` holds the grid waterfall; `findings/grid-target-control.md`. **Noiseless resolution sweep 2026-09-10** (`findings/probe-target-type.md`): grid-8x4 0.71 / 0.89, PI +0.27 / 1.05, ND +0.34 / 0.92, GS +0.27 / 1.11; grid-32x16 0.20 / 0.44, +0.17 / 1.13, +0.34 / 0.92, +0.29 / 0.90; appearance-lat (233 cells) 0.04 / 0.63, +0.04 / 1.68, **ND +0.51 / 0.79**, +0.30 / 1.08 — decodable at finer grids than 8-ray, editability flat regardless; grid-64x32 stopped unfitted. Blocks in `scores.json["bases"]`, probes in `probes/`. |
| `ray_ablation/L-dw-5ray-20m` | Transformer-L (regression) | dw-5ray | 780k steps, matched recipe — the quantisation push (launched 2026-09-10 night, unit `dw_5ray`, `scripts/drivers/dw_5ray.sh`; `Linear(5, 512)` in/out). Scored canonically + `appearance-fac` (stage E). Best val MSE 0.00694 at 450k (473 min). Canonical frustum: skill 0.94 / 0.97 (rand-init 0.95 / 0.96), PI +0.30 / 1.12, GS −0.11 — inert. **`appearance-fac` (9 + 4 classes, 52 logits): skill 0.93 / 0.93, PI +0.49 / 0.68, ND +0.56 / 0.90, GS +0.58 / 0.47 — above dw-8ray on every editor (+0.41 / +0.50 / +0.46); Sevan's prediction held.** Joint-cell `appearance` (14 × 3): skill 0.88 / 0.89, PI +0.54 / 0.78, ND +0.47 / 0.85, **GS +0.64 / 0.40** (8-ray: +0.43 / +0.43 / +0.61) — the bump replicates; highest discworld GS. No filtered selection file yet (canonical rows use the first 192 cases). `findings/probe-target-type.md`. |
| `ray_ablation/L-dw-128ray-20m` | Transformer-L (regression) | dw-128ray | IN FLIGHT (2026-09-15 20:57 PT → lab box, unit `dw_128ray`, driver `scripts/drivers/dw_128ray.sh`): 780k steps, matched recipe — the ray axis closed at the top. Scored canonically in BOTH bases (`PIM_DW_BASES=frustum,cartesian`) + `appearance-fac` (`dw_extra_targets`); listed in both table notebooks. |
| `smooth_ablation/L-dw-smooth-20m` | Transformer-L (regression) | dw-smooth | 780k steps, matched recipe — the anti-aliasing instance (stage 7 of the 2026-09-12 overnight chain, unit `rescore_protocol`, `scripts/drivers/rescore_2026-09-12.sh`; 516 min). Best val MSE 5.9e-5 at 735k (dw-noiseless 1.06e-3; Bayes floor 0 for both). Scored 2026-09-13 under the unified protocol (own 1000-case bench, every teleport ≥ 9 rays). Canonical frustum: skill 0.98 / 1.00 (rand-init 0.83 / 0.98, observation 0.23 / 0.95), **PI +0.11 / fid 2.62, ND −0.09, GS −0.02 — less editable than dw-noiseless (+0.23 / 1.54)**. `appearance-fac` (233 + 29 classes): skill 0.55 / 0.68, PI 0.00, **ND +0.38 / 0.94, GS +0.32 / 0.80** (noiseless +0.61 / +0.33). Anti-aliasing predicts better and edits worse. `findings/smooth-ablation.md`. Long-list tables only. |
| `observer_ablation/L-dw-8ray-obs5-20m` | Transformer-L (regression) | dw-8ray-obs5 | 780k steps, matched recipe — the multi-observer control (unit `dw_8ray_obs5`, `scripts/drivers/dw_8ray_obs5.sh`, 2026-09-13 17:50 → 09-14 07:29; `Linear(40, 512)` in/out; 476 min). Best val MSE 0.00302 (dw-8ray 0.00574). Scored 2026-09-14 under the unified protocol on its own 1000-case bench. **Cartesian** regression block: skill 0.98 / 1.00 (rand-init 0.93 / 0.99, observation 0.42 / 0.96), PI +0.19 / fid 1.60, ND −0.13, GS −0.07 — inert. **Per-view `appearance-fac`** (110 classes, 20 tiles): skill 0.96 / 0.99 (rand-init 0.91 / 0.98, observation 0.73 / 0.98), **PI +0.16 / 1.52, ND +0.42 / 1.06, GS +0.37 / 0.62 — below dw-8ray on every editor (+0.38 / +0.49 / +0.46), on dw-smooth's numbers.** `findings/observer-ablation.md`. Long-list tables only. |
| `ray_ablation/L-dw-8ray-20m` | Transformer-L (regression) | dw-8ray | 780k steps, matched recipe — the ray-count ablation (2026-09-04); the same 8 × 512 stack with `Linear(8, 512)` in/out (25.25M params). Best val MSE 0.00575. Driver `scripts/drivers/dw_8ray.sh`; findings `findings/ray-ablation.md`. Its `probes/` also holds the 18 INLP cascades of `experiments/inlp/8ray` (`findings/inlp-8ray.md`) Rescored 2026-09-09 on the filtered bench: unedited −0.911, **PI +0.282 / fid 0.90** (was +0.297 / 1.11 — now below the guard, i.e. non-destructive), ND −0.148, GS −0.064. Conclusion unchanged. **Probe-target sweep 2026-09-09→10** (`findings/probe-target-type.md`): under the observation-exact `appearance` target (30 cells) this run IS editable — GS **+0.614 / fid 0.39**, PI +0.43 / 0.76, ND +0.43 / 0.91, at every residual point, persisting over the rollout (waterfall `figures/waterfall_edits_appearance.png`); the sweep (`appearance-lat`, `-d2`, `-d3`; product grids `grid-4x2` … `grid-32x16`; misaligned 30-cell `grid-6x5`, `grid-10x3`) is unimodal at the exact partition — every product grid sits at GS +0.31…+0.37 regardless of resolution, the misaligned 30-cell controls at +0.34 — and `grid-16x8` here reproduces the noiseless grid's weak numbers. Blocks in `scores.json["bases"]`, probes in `probes/`. |
| `ray_ablation/R-dw-8ray-20m` | Recurrent-L (regression) | dw-8ray | 780k steps, identical recipe (2026-09-04), the architecture pair of `L-dw-8ray-20m`. Best val 0.00595 at step **40k**, drifting to 0.00625 by the end (no divergence spike this time). Scored: PI +0.191 / fid 1.41, GS −0.61; decodability at the random-init floor. Driver `experiments/recurrent/drivers/recurrent.sh` with `TOPIC=ray_ablation` **Un-quarantined 2026-09-11 (`runs/MOVES.md`) for `appearance-fac`: skill 0.94 / 0.94 (= the transformer's), PI +0.25 / 0.73 at point 0 ONLY, ND +0.13, GS +0.07 — the factorised read-out that unlocks the transformer (+0.41 / +0.50 / +0.46) does NOT unlock the GRU; the categorical result is architecture-dependent.** `findings/probe-target-type.md`. |
...
| `adjacent_flip_ablation/L-oth-adjacent-flip-20m` | Transformer-L (tokens) | oth-adjacent-flip | 780k steps, identical to `L-oth-adjacent-20m` except the instance (recolouring back ON under adjacency placement; 2026-09-10 → 11, unit `oth_adjacent_flip` on the WSL remote's RTX 4090, 25.7 h, `experiments/adjacent_flip_ablation/drivers/oth_adjacent_flip.sh`). Bayes-optimal (CE 2.301 vs 2.295, legal mass 0.998); skill LIN 0.947 / MLP 0.970 ABOVE the observation floor 0.888 / 0.914 (colour is computed, not looked up — oth-adjacent sat on its floor); **partially editable**: ND +0.24 / fid 0.62, PI +0.17 / 0.85, GS +0.03 / 2.62 vs unedited −0.70 — clearly above oth-adjacent, a third of standard Othello. `mine_signed`: skill 0.70 / 0.84, ND +0.26 / 0.63, PI +0.17 / 0.59. `findings/adjacent-flip-ablation.md` (`observed`). |
| `dropout_ablation/L-oth-adjacent-nodrop-390k` | Transformer-L (tokens), **dropout 0** | oth-adjacent | 390k steps (2026-09-11 → 12 on the WSL remote, unit `oth_adjacent_nodrop`, 12.5 h; resumable — extend with `--resume --steps 780000`), identical to `L-oth-adjacent-20m` except `--dropout 0`. Best val 2.4357 (Bayes 2.433; the dropout run 2.435). The dropout ablation for the materialisation theory: colour copies per tile 283 / 227 / 187 / 161 / 150 / 148 / 146 / 162 (pts 1–8) vs 232 / 138 / 99 / 88 / 90 / 85 / 83 / 83 with dropout — MORE redundant without it; K ≤ 16 copy-edits inert; best guarded K-copy edit +0.450 / 0.37 (pt1, K=128). Canonical scores (master_eval on the remote, 2026-09-12): CE 2.436 (Bayes 2.433), legal mass 0.999; skill LIN 0.979 / MLP 0.981 (dropout run 0.988 / 0.990; observation floor 0.988); unedited -0.655; PI -0.302 / fid 1.07, ND +0.042 / 2.96, GS +0.001 / 7.76 (dropout run: PI −0.05 / 2.31, ND +0.12 / 1.05, GS +0.00 / 5.73); mine_signed skill 0.821 / 0.902, ND -0.029 / 2.66 — canonically NOT editable, as with dropout. `experiments/dropout_ablation/`; `findings/adjacent-flip-ablation.md` §Dropout ablation. |
| `dropout_ablation/L-oth-adjacent-nodrop-20m` | Transformer-L (tokens), **dropout 0** | oth-adjacent | 780k steps = the `-390k` run continued 390k → 780k with `--resume` under a new name (2026-09-12 → 13 on the WSL remote, unit `oth_adjacent_nodrop_ext`; the copy of the resumable state is in `runs/MOVES.md` there; interrupted once at 400k by a WSL package update and resumed exactly). Best val 2.4351 (Bayes 2.4326). INLP copies per tile 272 / 210 / 158 / 132 / 122 / 119 / 118 / 137 (pts 1–8; 390k: 283 / 227 / 187 / 161 / 150 / 148 / 146 / 162; dropout 0.1: 232 / 138 / 99 / 88 / 90 / 85 / 83 / 83) — the tail prunes 10–20 % with doubled training and stays 1.2–1.6× the dropout run's; K ≤ 16 copy-edits inert, best guarded +0.422 / 0.42 (pt1, K=128). Canonical scores at eval 2026-09-12.1 (lab rescore): CE 2.4350, legal mass 1.000; skill LIN 0.973 / MLP 0.978; unedited −0.660; no positive guarded arm (PI −0.343 / 1.07, ND −0.206 / 1.04, GS −0.471 / 1.03; unguarded bests +0.002 / +0.043 / +0.003) — canonically NOT editable. (Remote scoring at eval 2026-09-01.4: PI −0.206 / 1.22, ND +0.053 / 2.95.) `experiments/dropout_ablation/`; `findings/adjacent-flip-ablation.md` §Dropout ablation → Extension to 780k. |
| `dropout_ablation/L-oth-adjacent-drop03-390k` | Transformer-L (tokens), **dropout 0.3** | oth-adjacent | 390k steps (2026-09-13 03:22 → 17:46 on the WSL remote, unit `oth_adjacent_drop03`, 13.3 h; resumable), identical to `L-oth-adjacent-20m` except `--dropout 0.3`. Best val 2.4354 at 385k (Bayes 2.4326) — the same optimum as dropout 0 / 0.1. Rescored on the lab box at eval 2026-09-12.1: legal mass 1.000; skill LIN 0.988 / MLP 0.991; unedited −0.641; **PI +0.172 / fid 1.04 (pt 1, α20) and ND +0.159 / 0.73 (pt 1, α1.5) — the first oth-adjacent run on which canonical editors land inside the guard** (symdiff +0.307 / +0.286); GS inert (−0.357 / 1.10). INLP copies per tile 185 / 137 / 115 / 101 / 97 / 90 / 87 / 86 (pts 1–8; plateau-then-cliff cascades); best guarded K-copy edit +0.445 / 0.52 (pt 3, K=64). Residual covariance participation ratio at pt 1: 84 (dropout 0: 44; 0.1: 81). `experiments/dropout_ablation/`; `findings/adjacent-flip-ablation.md` §Dropout ablation → Dropout 0.3. |
| `dropout_ablation/L-oth-adjacent-drop07-390k` | Transformer-L (tokens), **dropout 0.7** | oth-adjacent | 390k steps (2026-09-13 19:41 → 09-14 09:37 on the WSL remote, unit `oth_adjacent_drop07`, 13 h; resumable), identical to `L-oth-adjacent-20m` except `--dropout 0.7`. Val plateaued at 2.66–2.75 for ~110k steps (train/eval mismatch at p=0.7), then fell; best val **2.4403** at 360k — 0.008 above the Bayes floor (other arms ≤ 0.003). Rescored on the lab box at eval 2026-09-12.1: legal mass 0.998; skill LIN 0.981 / MLP 0.986; unedited −0.555; PI guarded −0.168 / 0.95 (unguarded +0.060 / 1.93), ND +0.033 / 0.98, GS inert — editability back down from the 0.3 arm (inverted U in the rate). INLP copies 169 / 126 / 112 / 106 / 98 / 93 / 94 / 93 (pts 1–8; point-1 initial R² 0.72), best guarded K-copy edit +0.391 / 0.69 (pt 4, K=64; point 1 never lands). Residual PR pt 1 76, pts 4–8 88–96. `experiments/dropout_ablation/`; `findings/adjacent-flip-ablation.md` §Dropout ablation → Dropout 0.7. |
| `blink_ablation/L-dw-blink-20m` | Transformer-L (regression) | dw-blink | 780k steps, matched recipe — the blink ablation (launched 2026-09-07 18:46, unit `dw_blink`, `scripts/drivers/dw_blink.sh`). The one discworld instance where position MUST be carried; scored canonically (first 192 edits) and by blink subset in `experiments/blink_ablation/scores/` (reappearance with staleness k, mid-blackout at the reappearance step, visible control; hidden-vs-visible decodability). **Position is carried (MLP 0.97–0.99 / LIN 0.63–0.89 on hidden frames, vs a 0.35 linear observation floor) and still NOT editable**: reappearance PI +0.27 / fid 1.37 (≈ +0.23 at fid ≤ 1.1) vs visible +0.22 / 1.81; GS ≤ 0 everywhere; mid-blackout edits do nothing at reappearance (−0.385 = unedited) unless destructive (fid 44). Canonical: LIN 0.90 / MLP 0.99, PI +0.22 / 1.78, GS −0.09. **`appearance-fac` (2026-09-10): ND +0.53 / 0.94, GS +0.33 / 0.95 — edits, but no better than noiseless under the same target (+0.63).** `findings/blink-ablation.md`. |
