...
| `pair_ablation/L-dw-pair-20m` | Transformer-L (regression) | dw-pair | IN FLIGHT (2026-09-15 evening → on the WSL remote, unit `dw_pair`, driver `scripts/drivers/dw_pair.sh`): 780k steps, matched recipe. Scored on the remote at the current eval version in both bases (`dw_bases_by_instance`) + `appearance-fac`; pulled back without the 410 GB corpus. |
