...
| **`<partition>-fac`** (factorised categorical; e.g. `appearance-fac`) | discworld | classification, OBJECT-indexed: per object one softmax per FACTOR of the partition — appearance: (run centre, run length) = 15 + 5 classes on dw-8ray; a product grid: (lateral bin, depth bin) — N_OBJ × F tiles over one shared class axis (each factor's classes offset, so a class index means one thing and the pooled majority reads as on Othello) | `grid_target.FactorisedTarget`; labels `factor_labels`, edit `edit_moves` → `Bench.moves` {tile, old, new}; PI swaps old ↔ new at every moved tile (`swap_class_logits`), ND adds the summed row contrast (`arms.categorical_direction`), GS asks CE toward the new labels; bench = the partition's cell-changing cases (`selection_target`) | `GRID_PROBE_RECIPE` (250k corpus, 200k seq, 50 epochs, streamed), fitted by `scripts/drivers/probe_target_fit.sh` (model + both floors) | `ray_ablation/L-dw-8ray-20m` (`appearance-fac`, 2026-09-10): skill 0.94 / 0.94 (rand-init 0.92 / 0.94, obs-right 0.48 / 0.94), **PI +0.41 / 0.71, ND +0.50 / 0.86, GS +0.46 / 0.53** vs the joint cell's +0.43 / +0.43 / +0.61 — the categorical gain survives factorisation (GS loses the joint move); the form to scale to 128 rays. `findings/probe-target-type.md` |
