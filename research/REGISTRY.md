...
| `dropout_ablation/L-oth-adjacent-drop07-390k` | Transformer-L (tokens), **dropout 0.7** | oth-adjacent | IN FLIGHT (2026-09-13 19:41 → on the WSL remote, unit `oth_adjacent_drop07`, driver `experiments/dropout_ablation/drivers/oth_adjacent_drop07.sh`): 390k steps, resumable, identical to `L-oth-adjacent-20m` except `--dropout 0.7`. Fourth dropout arm (Sevan): does the monotone trend (fewer point-1 colour copies, editors landing) continue, and where is the Bayes optimum lost? Remote scores land at the remote's eval 2026-09-01.4 — RESCORE on the lab box (`score_pending.sh`) before comparing. |
