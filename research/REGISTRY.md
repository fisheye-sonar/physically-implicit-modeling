...
| `dropout_ablation/L-oth-adjacent-nodrop-390k` | Transformer-L (tokens), **dropout 0** | oth-adjacent | PLANNED 2026-09-11 (launched on the WSL remote, unit `oth_adjacent_nodrop`, `experiments/dropout_ablation/drivers/oth_adjacent_nodrop.sh`): identical to `L-oth-adjacent-20m` except `--dropout 0` and 390k steps, trained RESUMABLE (`scripts/train.py --resume`, 2026-09-11) so it can be extended to 780k. The dropout ablation for the materialisation theory: does the ~85-copy fused colour code (and its inertness to single-probe edits) survive without the regulariser that rewards redundant writing? `experiments/dropout_ablation/`. |
