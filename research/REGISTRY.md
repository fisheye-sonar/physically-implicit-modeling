...
| `observer_ablation/L-dw-8ray-obs5-20m` | Transformer-L (regression) | dw-8ray-obs5 | 780k steps, matched recipe — the multi-observer control (unit `dw_8ray_obs5`, `scripts/drivers/dw_8ray_obs5.sh`, launched 2026-09-13 17:55; `Linear(40, 512)` in/out). **In flight**: scored on the Cartesian regression block + per-observer `appearance-fac` when the chain completes (~2026-09-14 morning). Long-list tables only. |
